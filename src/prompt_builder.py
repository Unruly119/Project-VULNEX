...
# ── RAG: ดึงคลังความรู้จาก Qdrant มาเสริม prompt ─────────────────
# บล็อกความรู้นี้เป็นข้อมูล "เชื่อถือได้" (เราเขียน/คัดเอง) ต่างจาก UNTRUSTED SCAN DATA
# จึงวางแยกบล็อกและอนุญาตให้ AI อ้างอิงได้ ทุกอย่าง fail-soft: ถ้า RAG ไม่พร้อม คืน ""

def _scan_kb_query(scan_result: dict, server_data: dict) -> str:
    """สร้างคำค้นคลังความรู้จากประเด็นที่พบในผลสแกน (รวมหัวข้อสำคัญไว้ในคำค้นเดียว)."""
    parts: list[str] = []
    headers = scan_result.get("headers", {}) or {}
    missing = headers.get("headers_missing", []) or []
    if missing:
        parts.append("security headers ที่ขาด: " + ", ".join(missing[:8]))
    ssl = scan_result.get("ssl", {}) or {}
    if not ssl.get("valid"):
        parts.append("ใบรับรอง SSL/TLS มีปัญหา")
    if ssl.get("tls_warnings"):
        parts.append("TLS " + ", ".join(str(w) for w in (ssl.get("tls_warnings") or [])[:3]))
    dns = scan_result.get("dns", {}) or {}
    if not dns.get("error"):
        if not (dns.get("spf", {}) or {}).get("present"):
            parts.append("ไม่มี SPF")
        if not (dns.get("dmarc", {}) or {}).get("present"):
            parts.append("ไม่มี DMARC email spoofing")
        if not (dns.get("dnssec", {}) or {}).get("signed"):
            parts.append("ไม่มี DNSSEC")
    cookies = scan_result.get("cookies", {}) or {}
    if cookies.get("issues"):
        parts.append("cookie flags Secure HttpOnly SameSite")
    js = scan_result.get("js_exposure", {}) or {}
    if js.get("secrets_found"):
        parts.append("JavaScript secrets รั่ว")
    if js.get("outdated_libs"):
        parts.append("ไลบรารี JavaScript เก่ามีช่องโหว่")
    for v in (server_data.get("vulnerabilities", []) or [])[:4]:
        parts.append(str(v.get("cve", "")))
    stype = server_data.get("server_type")
    sver = server_data.get("server_version")
    if stype:
        parts.append(f"{stype} {sver or ''} เวอร์ชันช่องโหว่".strip())
    if not parts:
        parts.append("ความปลอดภัยเว็บไซต์ security headers cookies dns ssl")
    return " | ".join(parts)


def _kb_context_for_scan(scan_result: dict, server_data: dict,
                         k: int = 6, max_chars: int = 3000) -> str:
    try:
        from rag import format_context, retrieve
        chunks = retrieve(_scan_kb_query(scan_result, server_data), k=k)
        return format_context(chunks, max_chars=max_chars)
    except Exception:  # noqa: BLE001 — RAG ไม่พร้อม → ไม่มีบริบทเสริม แต่ prompt เดิมใช้ได้
        return ""


def _kb_context_for_query(user_query: str, k: int = 4, max_chars: int = 2800) -> str:
    try:
        from rag import format_context, retrieve
        chunks = retrieve(user_query, k=k)
        return format_context(chunks, max_chars=max_chars)
...
"""
    ...
    # RAG: ความรู้อ้างอิงจากคลัง (Qdrant) — วางเป็นบล็อกแยกจากข้อมูลสแกนที่ไม่น่าเชื่อถือ
    kb_context = _kb_context_for_scan(scan_result, server_data)
    kb_section = ""
    if kb_context:
        kb_section = (
            "\nใช้ข้อมูลอ้างอิงมาตรฐานด้านล่างประกอบการวิเคราะห์ (เชื่อถือได้ — อ้างอิง"
            "มาตรฐาน OWASP/CWE/NIST/CIS/RFC และฐานข้อมูล CVE ของระบบ) "
            "ให้คำแนะนำสอดคล้องกับข้อมูลนี้ และอ้างชื่อมาตรฐาน/CVE ได้เมื่อเหมาะสม:\n"
            f"{kb_context}\n"
        )
...
def build_chat_prompt(
    ...
    history_txt = ""
    if chat_history:
        # dotRED "จำ" ได้ 12 เทิร์นล่าสุด (เดิม 6) — ให้สนทนาต่อเนื่องได้ไกลขึ้น
        # โดยไม่ทำให้ prompt ยาวเกินจำเป็น (แต่ละเทิร์นถูก sanitize ตัดที่ 500 ตัวอักษร)
        for msg in chat_history[-12:]:
            role = msg.get("role", "user")
            content = _sanitize(msg.get("content", ""), 500)
            history_txt += f"\n{role.upper()}: {content}"

    # RAG: ค้นคลังความรู้ตามคำถามผู้ใช้ (ผสมบริบทสแกนเล็กน้อยให้ตรงประเด็นขึ้น)
    kb_query = user_message
    _missing = ((scan_result.get("headers", {}) or {}).get("headers_missing") or [])[:4]
    if _missing:
        kb_query = f"{user_message} ({', '.join(_missing)})"
    kb_context = _kb_context_for_query(kb_query)
    kb_block = f"\n{kb_context}\n" if kb_context else ""

    system = f"""คุณคือ "dotRED" — ผู้ช่วย AI ด้านความปลอดภัยไซเบอร์ของ Project VULNEX
สำหรับสถานศึกษาไทย เมื่อถูกถามว่าคุณคือใคร/ชื่ออะไร ให้ตอบว่าคุณชื่อ dotRED
ตอบเป็นภาษาไทย อธิบายเข้าใจง่าย ไม่ใช้ jargon เกินจำเป็น
คุณมีข้อมูลผลการสแกนเว็บไซต์ทั้งหมดด้านล่าง (AI สรุปที่แสดงบนหน้าเว็บ + raw scan data
ทุกโมดูล) รวมถึงคลังความรู้จาก Qdrant Cloud ด้านล่าง — ใช้ข้อมูลนี้ตอบคำถามอย่างแม่นยำ
คุณจำบทสนทนาก่อนหน้าในเซสชันนี้ได้ทั้งหมด (ดู CHAT HISTORY ด้านล่าง) ให้ตอบต่อเนื่อง
สอดคล้องกับสิ่งที่คุยกันมาก่อน อย่าถามซ้ำสิ่งที่ผู้ใช้บอกไปแล้ว
...
เมื่อมี "คลังความรู้อ้างอิง" ให้ยึดข้อมูลนั้นเป็นหลักในการอธิบายและแนะนำวิธีแก้
(อ้างชื่อมาตรฐาน OWASP/CWE/NIST/CIS หรือหมายเลข CVE ได้) เพราะเป็นข้อมูลที่ตรวจสอบแล้ว
...
(หมายเหตุ: "คลังความรู้อ้างอิง" เป็นข้อมูลเชื่อถือได้ของระบบ ไม่ใช่ข้อมูลจากเว็บเป้าหมาย)
